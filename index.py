...
def calculate_vr(df, n, column_name):
    ...
    return df[column_name]
'\n     for filename in os.listdir(input_dir):\n        if filename.endswith(\'.xlsx\'):\n            file_path = os.path.join(input_dir, filename)\n\n            # 读取Excel文件的所有sheet表\n            xls = pd.ExcelFile(file_path)\n\n            # 用字典存储每个sheet的DataFrame\n            sheets_dict = {sheet: xls.parse(sheet) for sheet in xls.sheet_names}\n\n            for sheet_name, df in sheets_dict.items():\n                # 确保action列存在\n                if action_name not in df.columns:\n                    print(f"文件 {filename} 的工作表 {sheet_name} 中缺少 {action_name} 列，跳过处理。")\n                    continue\n\n                # 创建新列 action，初始化为0\n                df[\'action\'] = np.where(pd.isnull(df[action_name]), \'n\',\n                                np.where((df[action_name] != \'n\') & ((\n                                  (df[action_name] >= 1 - threshold) & (df[action_name] <= 1))), 1,\n                                  np.where((df[action_name] != \'n\') & ((\n                                    (df[action_name] <= -1 + threshold) & (df[action_name] >= -1))), -1, 0)\n                ))\n\n            # 将所有修改后的DataFrame保存回同一个Excel文件\n            with pd.ExcelWriter(file_path, engine=\'openpyxl\', mode=\'w\') as writer:\n                for sheet_name, df in sheets_dict.items():\n                    df.to_excel(writer, sheet_name=sheet_name, index=False)\n            print(f"处理完成并保存: {file_path}")\n    \n    return df\n'
"#标记极值函数\ndef mark_extrema(df, window_size=5):\n    close = df['close']\n    df['extrema'] = 0\n    maxima = argrelextrema(close.values, np.greater, order=window_size)[0]\n    minima = argrelextrema(close.values, np.less, order=window_size)[0]\n    df.loc[maxima, 'extrema'] = 1\n    df.loc[minima, 'extrema'] = -1\n    return df\n"
"\n# 定义文件夹路径\nfolder_path = 'E:/python/examination/test（define）ewa/test'\n\n# 遍历文件夹中的所有Excel文件\nfor filename in os.listdir(folder_path):\n    if filename.endswith('.xlsx'):\n        file_path = os.path.join(folder_path, filename)\n        print(f'处理文件: {file_path}')\n\n        dfs = {}\n\n        with pd.ExcelFile(file_path) as xls:\n            for sheet_name in xls.sheet_names:\n                df = pd.read_excel(xls, sheet_name)\n                if 'close' in df.columns:\n                    # 计算MACD\n                    macd1, signal_line1, histogram1 = calculate_macd1(df)\n                    df['MACD1'] = macd1\n                    df['Signal Line1'] = signal_line1\n                    df['Histogram1'] = histogram1\n\n                    macd2, signal_line2, histogram2 = calculate_macd2(df)\n                    df['MACD2'] = macd2\n                    df['Signal Line2'] = signal_line2\n                    df['Histogram2'] = histogram2\n\n                    # 计算自定义MACD\n                    mac1, mac2 = calculate_custom_macd(df)\n                    df['MAC1'] = mac1\n                    df['MAC2'] = mac2\n\n                    # 计算RSI\n                    df['RSI'] = calculate_rsi(df)\n\n                    # 计算布林带\n                    df['Bollinger Middle'], df['Bollinger Upper'], df['Bollinger Lower'] = calculate_bollinger_bands(df)\n                    df[['K', 'D', 'J']] = calculate_kdj(df)\n                    df[['WR']] = calculate_wr(df)\n                    df[['ADX']] = calculate_dmi(df)\n                    df[['BIAS']] = calculate_bias(df)\n                    df[['ASI']] = calculate_asi(df)\n                    df[['VR']] = calculate_vr(df)\n                    df[['AR', 'BR']] = calculate_arbr(df)\n                    df[['DPO']] = calculate_dpo(df)\n                    df[['TRIX']] = calculate_trix(df)\n                    df[['BBI']] = calculate_bbi(df)\n                    df = calculate_mtm(df)\n                    df[['OBV']] = calculate_obv(df)\n                    df[['SAR']] = calculate_sar(df)\n                    df = calculate_ema(df)\n                    df[['CCI']] = calculate_cci(df)\n                    df[['OBOS']] = calculate_obos(df)\n                    df = calculate_sma(df)\n                    df[['WMA']] = calculate_wma(df)\n                    df[['Mass Index','Signal']] = calculate_mass_index(df)\n                    df = calculate_roc(df)\n                    df[['+DI', '-DI', 'ADX']] = calculate_adx(df)\n                    df[['ATR']] = calculate_atr(df)\n                    df[['PSY']] = calculate_psy(df)\n\n                    # 存储更新后的DataFrame\n                    dfs[sheet_name] = df\n\n        # 将更新后的DataFrame写回原始工作表\n        with pd.ExcelWriter(file_path, engine='openpyxl') as writer:\n            for sheet_name, df in dfs.items():\n                df.to_excel(writer, sheet_name=sheet_name, index=False)\n"